...
/*
 *  > 100 -- batch size for reclaiming locks
 *  < 100 -- percentage based on total granted locks
 */
 ...
 */
static inline s64 ldlm_reclaim_period(s64 pressure_scaled)
{
 ...
 */
static int ldlm_reclaim_client_notify_ast(struct obd_export *exp,
    struct ldlm_reclaim_info *info, struct ptlrpc_request_set *req_set)
{
	int rc = 0;
...
static void ldlm_reclaim_ns(int total_count)
{
	...
	if (ldlm_reclaim_batch < 100) {
		count = (total_count * ldlm_reclaim_batch) / 100;
